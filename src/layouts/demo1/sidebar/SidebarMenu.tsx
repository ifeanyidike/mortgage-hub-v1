...
const SidebarMenu = () => {
  ...
  const buildMenu = (items: TMenuConfig) => {
    return MENU_SIDEBAR.map((item, index) => {
      if (item.heading) {
  ...
  const buildMenuArrow = () => {
    return (
      ...
      >
        ...
        <span className="hidden menu-item-show:inline-flex">
          <KeenIcon icon="minus" className="text-2xs" />
        </span>
      </MenuArrow>
    );
  };
  ...
  return (
    ...
    >
      {MENU_SIDEBAR && buildMenu(MENU_SIDEBAR)}
    </Menu>
  );
};
